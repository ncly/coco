...
createData_SA_x0_sigma <- function(x0_min, x0_max, sigma_min, sigma_max){
  ...
  for(x0_increment in seq(from=x0_min, to=x0_max, by=((x0_max-x0_min)/10)))
  {
    for(sigma_increment in seq(from=sigma_min, to=sigma_max, by=((sigma_max-sigma_min)/10)))
    {
      ...
      data[counter, 2] <- price_coco_sa(T <- 10, nsimulations <- 10000, rho <- - 0.2, kappa <- 0.114, r_bar <- 0.069, r0 <- 0.01, sigma_r <- 0.07, mu_Y <- -0.01, sigma_Y <- 0.02, lambda <- 1, g <- 0.5, x_hat <- 1.1, b0 <- 0.04, p <- 1, e_bar <- 0.02, sigma_x <- 0.02, x0 <- 1.15, B <- 1, coupon <- 0.06)
      data[counter, 3] <- sigma_increment
      counter <- counter + 1
    }
  }
  write.table(data, file = "createData_SA_x0_sigma_2.txt", row.names = FALSE, quote=FALSE)
}
...
createData_SA_T_r <- function(T_min, T_max, r_min, r_max){
  ...
  write.table(data, file = "createData_SA_T_r_2.txt", row.names = FALSE, quote=FALSE)
}
...
createData_SA_x0_ebar <- function(x0_min, x0_max, ebar_min, ebar_max){
  ...
  write.table(data, file = "createData_SA_x0_ebar_2.txt", row.names = FALSE, quote=FALSE)
}
...
createData_SA_x0_lambda <- function(x0_min, x0_max, lambda_min, lambda_max){
  ...
  write.table(data, file = "createData_SA_x0_lambda_2.txt", row.names = FALSE, quote=FALSE)
}
...
createData_SA_x0_b0 <- function(x0_min, x0_max, b0_min, b0_max){
  ...
  write.table(data, file = "createData_SA_x0_b0_2.txt", row.names = FALSE, quote=FALSE)
}
